const STATUS_CONFIG = {
  ...
  AI_DONE:     { label: 'tara reviewed it',       color: 'bg-green-50 text-green-700', useBadgeIcon: true },
  DRAFT:       { label: 'Draft',                  color: 'bg-yellow-50 text-yellow-700' },
  SENT:        { label: 'Sent ✓',                 color: 'bg-green-50 text-green-700', useBadgeIcon: true },
}
...
export default function StatusBadge({ status, className = '' }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${cfg.color} ${className}`}>
      {cfg.useBadgeIcon ? (
        <img src="/badge-reviewed.svg" alt="" className="w-3 h-3" />
      ) : cfg.pulse ? (
