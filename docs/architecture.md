...
Currently implemented: `ClaudeProvider` and `CodexProvider`, both using shared prompts from `app/ai/prompts.py`.

Prompts are the natural language instructions sent to the AI. They are read at runtime from a shared prompt store (`data/prompts.json`) with hardcoded defaults. Users can edit prompts via the Instructions tab on the landing page (`GET/PUT/DELETE /api/prompts/{key}`). JSON schemas stay hardcoded — only the instruction text is editable.

The `repo_path` parameter sets the working directory for the subprocess, giving the AI access to read any file in the cloned repo. `CLAUDECODE` env var is stripped before each Claude call to allow nested sessions.

---

## Mermaid Diagram Rendering

AI output can include mermaid code fences (` ```mermaid `). The frontend renders these as inline SVG diagrams using the `mermaid` library. If the syntax is invalid, it falls back to a raw code block.

Components: `Mermaid.jsx` (renders a single diagram), `MarkdownWithMermaid.jsx` (wraps ReactMarkdown with a custom code block renderer that detects mermaid fences).

Applied to: PR summary, chunk assessment, chunk summary, AI suggestions, re-review summary, chat replies, thread discussions.

---

## Inline Draft Comment Indicators

Draft comments generated by the AI are shown as 💬 icons directly on the diff lines they reference. Clicking an icon expands the draft inline below the diff line — users can edit, send to GitHub, or delete the draft without switching to the right panel.

Data flow: `ChunkPanel` fetches drafts → passes to `DiffView` → groups by path → groups by line → renders `InlineDraftPopover` components with full edit/send/delete actions.
